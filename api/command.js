...
글자 교체 (목록 순서가 곧 결과의 읽기 순서):
${layerBlock}

전체 언어 지정(참고): ${instruction || '(개별 지정 우선)'}

지켜야 할 규칙:
- **스타일은 인식된 레이어를 따라간다**: 각 레이어의 글자 색, 그라데이션, 굵기, 외곽선, 그림자, 입체감, 광택은 그 레이어 원문 구간에서 보인 스타일 그대로다. 레이어 순서가 바뀌어도 주황 원문은 주황, 갈색 원문은 갈색을 유지한다. 자리를 옮겼다고 옆 레이어 색을 가져오지 마라. 없던 효과를 추가하거나 있던 효과를 빼지 마라.
