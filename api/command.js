...
규칙:
- 위 목록에 "사용자가 이미 수정 지정함"이라고 표시된 줄은 그 지정된 텍스트로 반드시 교체하라.
...
- 모든 텍스트(글자, 외곽선, 그림자 포함)는 이미지 가장자리(상하좌우)에서 안쪽으로 최소 3% 여백 안에 완전히 들어가야 한다. 만약 원래 텍스트 크기로는 이 여백을 넘어선다면, 절대 줄바꿈(개행)하지 말고, 다른 모든 스타일 효과는 그대로 유지한 채 글자 크기만 비례적으로 줄여서 한 줄로 여백 안에 맞춰라. 가장자리에 닿거나 잘리게 두지 마라.
- 원본에서 한 줄이던 텍스트는 결과에서도 반드시 한 줄로 유지하라. 원본에 없던 줄바꿈을 새로 만들지 마라.
- 배경, 장식, 색상, 조명 등 텍스트가 아닌 모든 요소는 원본과 100% 동일하게 유지하고 다시 그리지 마라.
- 결과 이미지는 입력 이미지와 동일한 구도로 출력하라.` }
            ]
          }],
          generationConfig: {
