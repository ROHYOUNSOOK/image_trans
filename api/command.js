...
- **텍스트 효과는 원본을 관찰해서 그대로 복사한다**: 특정 효과를 가정하지 말고, 입력 이미지에서 각 줄(또는 각 구간)이 실제로 가진 글자 색, 글자 두께(굵기)와 크기, 외곽선(테두리)의 색과 두께, 그림자, 입체감, 광택을 눈으로 확인한 뒤 새 글자에 똑같이 입혀라. 특히 원본에서 굵은(볼드) 부분은 결과도 굵게, 얇은 부분은 결과도 얇게 그 두께 대비를 정확히 유지하라. 원본에 없는 효과(예: 은색 크롬, 금속 질감)를 임의로 추가하지 마라. 반대로 원본에 있는 효과를 빼거나 밋밋한 단색·평범한 볼드로 단순화하지도 마라. 각 부분은 원본에서 가진 색·두께·외곽선을 정확히 그대로 물려받아야 한다.
