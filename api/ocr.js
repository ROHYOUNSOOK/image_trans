...
1) Different font COLOR — even subtle differences count (e.g. white vs yellow, gold vs orange, dark-red vs bright-red, or any noticeable hue/saturation/brightness shift). If two adjacent words are not the EXACT same color, they MUST be separate segments.
...
3) Different font TEXTURE or material (e.g. metallic/glossy text vs flat/matte text, outline-only vs filled, embossed/3D vs plain, gradient-filled vs solid). If parts of a line have visually different surface treatments or textures, split them into separate segments.
Do NOT merge parts with different color, weight, or texture into one box. Keep each segment's box tight around only that part.
Return each segment with a tight bounding box "box_2d" as [ymin, xmin, ymax, xmax] normalized to 0-1000, and the text as "label".` + (extra ? `\n\nADDITIONAL INSTRUCTIONS FROM USER:\n${extra}` : '') }
            ]
          }],
          generationConfig: {
            temperature: 0,
