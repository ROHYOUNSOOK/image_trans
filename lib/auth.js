...
export async function getAuth(req) {
  ...
  let user = data.users[sess.email] || null;
  if (!user) {
    user = {
      email: sess.email,
      name: sess.name || '',
      ...
      status: sess.status === 'approved' || sess.status === 'rejected' ? sess.status : 'pending',
      createdAt: nowKst(),
      lastLoginAt: nowKst()
    };
  }
  ...
  await updateStore(store => {
    store.users[user.email] = { ...(store.users[user.email] || {}), ...user };
    return store;
  });
