...
const BLOB_PATH = 'app-store.json';
const EMPTY = { users: {}, images: [] };
...
async function readRaw() {
  if (useBlob()) {
    try {
      ...
      if (!result || result.statusCode !== 200) return { ...EMPTY, users: {}, images: [] };
...
async function writeRaw(data) {
  const body = JSON.stringify(normalize(data), null, 2);
  if (useBlob()) {
